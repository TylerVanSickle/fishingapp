  ...
  return (
    <>
      <button
        ref={btnRef}
        onClick={openMenu}
        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-slate-200 hover:bg-white/5 transition-colors whitespace-nowrap"
      >
          ...
          <div className="flex flex-col min-w-45">
            {col1.map(({ href, label }) => (
              <Link
                ...
                className="flex items-center px-4 py-2.5 text-sm text-slate-400 hover:text-white hover:bg-white/6 transition-colors whitespace-nowrap gap-2"
              >
                {label.startsWith("✦") ? (
                  <>
