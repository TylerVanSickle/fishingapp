...
export default async function FollowingPage({
  ...
  type FollowedUser = { id: string; username: string; avatar_url: string | null; bio: string | null };
  const following: FollowedUser[] = (follows ?? [])
    .map((f) => f.profiles as unknown as FollowedUser | null)
    ...
    <div className="max-w-lg mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-lg font-bold text-white">
          Following
          <span className="ml-2 text-slate-500 font-normal text-base">{following.length}</span>
        </h1>
      </div>
